...
exports.createMeeting = asyncHandler(async (req, res,next) => {
  try {
    ...
    const {meetingTitle,participants,host,meetingDate} = req.body.joinData
    ...
    const meeting = await meetingModel.findById({_id:addMeeting._id}).populate({path:"host",select:["name","email"]}).populate({path:"participants",select:["name","email"]});
...
exports.meetings = asyncHandler(async (req, res) => {
  try {
    ...
    const apiFeature = new apiFeatures(meetingModel.find(),req.query).search()
    const meeting = await apiFeature.query.populate({path:"host",select:["name","email"]}).populate({path:"participants",select:["name","email"]});
...
exports.DetailMeeting = asyncHandler(async(req,res,next)=>{
  try {
    ...
    const meeting = await meetingModel.findById(id).populate({path:"host",select:"name"}).populate({path:"participants", select:["name"]});
...
exports.EditMeeting=asyncHandler(async(req,res,next)=>{
  try {
    ...
    meeting = await meetingModel.findByIdAndUpdate(id,{
...
exports.deActivate = asyncHandler(async(req,res,next)=>{
  try {
    ...
    const changeMeeting = await meetingModel.findByIdAndUpdate(id,{
